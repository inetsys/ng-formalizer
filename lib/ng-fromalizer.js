...
function createFieldConfiguration(definition, cfg, $scope, field_in_scope) {
    ...
    switch (cfg.type) {
    case "checkbox-list":
        // todo handle properly the model

        // ng-checked="id_list.indexOf(obj.id) > -1"
        // ng-click="toggleSelection(obj.id)"

        field.element.attrs.type = "checkbox";
        field.container["class"].push("checkbox");
        field.element.attrs["class"].splice(
            field.element.attrs["class"].indexOf("form-control"),
            1
        );


        break;
    case "typeahead-multi":
        var add_fn = camelCase("add-" + name),
        ...
        // p as p.name for p in products | filter:{name:$viewValue}

    // FALL THROUGH EXPECTED!
    case "typeahead":
        if (field.source_display) {
            field.element.attrs.typeahead = "p as p." + field.source_display + " for p in " + field_in_scope + ".source | filter:{" + field.source_display + ":$viewValue}";
        } else {
            field.element.attrs.typeahead = "for p in " + field_in_scope + ".source";
        }
...
function parseFields(definition, fields, new_fields,
    ...
    for (i = 0, max = fields.length; i < max; ++i) {
        ...
        var errs = $interpolate(
            $interpolate(error_list)(field).replace(/\{\\\{/g, "{{").replace(/\}\\\}/g, "}}")
        )(field);
...
function load_templates($q, $http, $templateCache) {
    return $q.all([
        $http
            .get("formalizer-error-list.tpl.html", {cache: $templateCache})
            .success(function (html) {
                error_list = html;
            }),
        $http
            .get("formalizer-input.tpl.html", {cache: $templateCache})
            .success(function (html) {
                templates.inputs = html;
            }),
        $http
            .get("formalizer-textarea.tpl.html", {cache: $templateCache})
            .success(function (html) {
                templates.textarea = html;
            }),
        $http
            .get("formalizer-checkbox.tpl.html", {cache: $templateCache})
            .success(function (html) {
                templates.checkbox = html;
            }),
        $http
            .get("formalizer-select.tpl.html", {cache: $templateCache})
            .success(function (html) {
                templates.select = html;
            }),
        $http
            .get("formalizer-submit.tpl.html", {cache: $templateCache})
            .success(function (html) {
                templates.submit = html;
            }),
        $http
            .get("formalizer-typeahead.tpl.html", {cache: $templateCache})
            .success(function (html) {
                templates.typeahead = html;
            }),
        $http
            .get("formalizer-checkbox-list.tpl.html", {cache: $templateCache})
            .success(function (html) {
                templates.checkbox_list = html;
            }),
